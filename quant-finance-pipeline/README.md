...
- **Module 01（当前）**：OPM 方法论与工程骨架
  - OPM 产物（系统上下文、数据生命周期、部署回滚、责任矩阵）
  - 数据契约（trade event JSON Schema）
  - OPM 一致性检查脚本
- Module 02：基础云资源 Terraform（GCP：GCS/BQ/PubSub）
- Module 03：实时采集链路（collector -> Pub/Sub）
- Module 04：流处理链路（Dataflow/Flink -> BigQuery）
- Module 05：批处理链路（Airflow -> GCS -> BigQuery）
- Module 06：dbt 分层建模与 alpha 因子
- Module 07：Streamlit 双 Tile 看板
